...
- Package manager is pnpm (`packageManager` pinned in `package.json`, lockfile `pnpm-lock.yaml`). Target scripts per README: `pnpm run dev|check|build|lint` (currently placeholder stubs, green until the real scaffold lands); pre-commit gate `pnpm run check && pnpm run build && pnpm run lint`. Do not invent others.
