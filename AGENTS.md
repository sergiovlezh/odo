...
- Triage labels: type (what it IS) vs state (where it STANDS). Templates auto-apply `bug`/`enhancement` + `needs-triage` to every new issue. Flow: `needs-triage` → read → `needs-info` (stuck, waiting on reporter) / `ready-for-agent` (fully specified, buildable alone) / `ready-for-human` (needs product call, judgment, phone test) / `wontfix` (out of scope, e.g. hybrids; close with reason). Filter `ready-for-agent` for "what can I build now".
